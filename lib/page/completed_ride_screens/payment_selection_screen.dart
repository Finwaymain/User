...
import 'package:finway/controller/payment_controller.dart';
...
import 'package:finway/model/xenditModel.dart';
...
class PaymentSelectionScreen extends StatelessWidget {
        ...
        return Scaffold(
          ...
          body: Stack(
            children: [
              ...
              Obx(() => controller.isSimulatingUPI.value
                  ? Positioned.fill(
                      child: Container(
  ...
  tipAmountBottomSheet(BuildContext context, bool isDarkMode, PaymentController controller) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFieldWidget(hintText: 'Enter Tip'.tr, controller: controller.tripAmountTextFieldController),
                    Padding(
                      padding: const EdgeInsets.only(top: 10),
                      child: Row(
                        children: [
  ...
  mercadoPagoMakePayment({required BuildContext context, required String amount, required UserModel user, required PaymentController controller}) async {
    final headers = {
  ...
  Future<XenditModel> createXenditInvoice({required var amount, required PaymentController controller}) async {
...
//Orangepay payment
  ...
  orangeMakePayment({required String amount, required BuildContext context, required PaymentController controller}) async {
    reset();
  ...
  Future fetchToken({required String orderId, required String currency, required BuildContext context, required String amount, required PaymentController controller}) async {
  ...
  Future webpayment(
      {required String orderIdData, required BuildContext context, required String currency, required String amountData, required PaymentController controller}) async {
    orderId = orderIdData;
    amount = amountData;
...
//Midtrans payment
  midtransMakePayment({required String amount, required BuildContext context, required PaymentController controller}) async {
  ...
  Future<String> createPaymentLink({required var amount, required PaymentController controller}) async {
  ...
  void _showWalletMpinDialog(BuildContext context, PaymentController controller, List taxList) {
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Icon(Icons.lock_outline, size: 36, color: AppThemeData.primary200),
            const SizedBox(height: 12),
            Text(
              "Enter Security MPIN".tr,
              style: const TextStyle(
                fontSize: 18,
                fontFamily: AppThemeData.semiBold,
              ),
            ...
            Text(
              "Enter your MPIN to authorize wallet payment of ${Constant().amountShow(amount: controller.getTotalAmount().toString())}".tr,
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 13,
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            if (pin.isEmpty) {
                              errorMessage.value = "Please enter your MPIN".tr;
                              return;
                            }
                            ...
                            try {
                              ...
                              if (body['valid'] == true || body['success'] == 'success') {
                                Get.back(); // Close MPIN sheet
                                Map<String, dynamic> bodyParams = {
                                  'id_ride': controller.data.value.id.toString(),
                                  'id_driver': controller.data.value.idConducteur.toString(),
                                  'id_user_app': controller.data.value.idUserApp.toString(),
                                ...
                                errorMessage.value = body['message'] ?? 'Incorrect MPIN. Please try again.'.tr;
                              }
                            } catch (e) {
                              isVerifying.value = false;
                              errorMessage.value = 'Failed to verify MPIN: $e';
                            }
                          },
                          style: ElevatedButton.styleFrom(
                            backgroundColor: AppThemeData.primary200,
